...
for (const [group, items] of groups) {
  ...
  for (const variants of items) {
    ...
    body += variants.map(v=>`<li>${escape([v.size,v.color,v.variant].filter(Boolean).join(' · ') || 'See retailer for size details')} — ${v.price===null?'Price unavailable':escape(money.format(v.price))}</li>`).join('');
    body += '</ul>';
