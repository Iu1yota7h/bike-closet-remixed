...
| Sitemaps / robots | Public robots allows crawling and points to sitemap.xml. Sitemap contains only the homepage; filtered combinations are excluded. Missing URLs return 404. Prior Google/Bing submissions are recorded in task history; current Search Console/Bing indexing and processing were not independently rechecked. |
| LLM discovery | Public CSV and JSON stock data remain available without executing the filter app. A small llms.txt navigation guide is generated from the site origin. This is a convenience file, not a guarantee that any model will read or cite the site. robots currently permits both search and training crawlers; no new exclusion policy was added. |
