...
Set `SITE_URL` to the final HTTPS origin (for example `https://bikes.example.com`, without paths) in the hosting build environment. This enables the homepage canonical and a sitemap containing only the homepage. Without it, no placeholder domain or sitemap is published. Filter URLs share the homepage canonical; filter combinations are excluded from the sitemap. After launch, submit `/sitemap.xml` to Search Console. Linked JSON/CSV help discovery; they do not guarantee search rankings or AI citations.
