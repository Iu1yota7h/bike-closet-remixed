...
The daily inventory refresh is the only review-research trigger. It compares parent product IDs with the prior validated snapshot and adds each genuinely new, in-stock parent to `data/research-queue.json`. A first snapshot is a baseline, not 500 arrivals. The durable `pending` list keeps every arrival; `shortlist` exposes at most five models per research execution. Existing parent IDs, new variants, restocks and price changes do not create work. Products already recorded in `public/research.json` or the completion ledger are removed from the queue, including inconclusive searches. The inventory job performs no web search or AI call; the queue is processed only when a review agent is explicitly run for those arrivals. There is no weekly review schedule.
...
## Public copy and review panels

Owner direction: write like the personal intro—short, plain, conversational and useful. Avoid research-process jargon, generic buying advice and placeholder panels. Only sourced reviews qualify for a Reviews button or published product notes. Do not show Specs & Fit panels, failed-search explanations, editorial tier labels or generic compatibility guidance. Keep source-backed findings and tradeoffs in short summaries with links; never imply the owner personally tested the product. Preserve saved evidence and completion records internally so hiding low-value content does not trigger repeat research. Apply this to the interactive and static catalogs.

Filter options must have matching in-stock data within the other active filters. A picker excludes its own selection when deriving choices; pickers that clear size also ignore the current size. Always keep the All/reset choice. Do not offer an empty Unspecified brand bucket; retire old brand=unknown links when the full stock snapshot contains no unknown brands. Show a simple no-results message instead of an empty table and pagination.

Responsive browsing: keep the sortable table on desktop and labeled product cards at 900 CSS pixels and below. On smaller screens, collapse the filter panel, keep removable selections visible, and provide a sort picker because table headings are hidden. Group and product-type labels are instant filter shortcuts. Preserve the same filtered rows, exact retailer details, reviews, shop links and pagination in both layouts. Check narrow phones, tablets, desktop and zoom-equivalent widths when changing this layout.

Site voice: write in the owner's personal, simple style. Use I for this project and you for the reader. Keep controls short (Everything, Has a review, On sale), explain discounts as Bike Closet's list-price comparison, and use concrete review takeaways instead of formal ratings such as Positive review or caveat. Attribute findings to their source; never imply the owner tested a product. Keep test-model differences, limitations, source links and original research dates when simplifying a summary. Apply the same voice to the home page, mobile cards, About page, static catalog, metadata and future review notes.

Quality process: retain readable HTML discovery alongside interactive filters; preserve metadata, canonical and sitemap origin. Serve compact review data derived from the same saved sources. Keep mutable data revalidated and only fingerprinted /_next/static files immutable. Run scripts/test-quality.mjs after a build; don't weaken size budgets without explaining measured growth. Recheck responsive controls after changing the explicit Tailwind source list. Keep status tied to the deployed data snapshot, and distinguish measured browser results from untested device or field metrics.
